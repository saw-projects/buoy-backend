...
from config import MAX_CHAR_COUNT, MIN_CHAR_COUNT, TEST_USER
...
async def process_query(request: Request, query: QueryRequest = None, user_id: int = Depends(get_current_user)):
    ...
    """
    try:
        # User is already validated through the token
        # validate query
...
async def job_status(job_id: str, user_id: int = Depends(get_current_user)):
    ...
    try:
        ...
        if db.job_exists(job_id=job_id):
            ...
            # TODO: Add check to verify the job belongs to the authenticated user
